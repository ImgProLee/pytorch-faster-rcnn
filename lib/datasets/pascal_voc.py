...
class pascal_voc(imdb):
    def __init__(self, image_set, year, devkit_path=None):
        ...
        self._data_path = os.path.join(self._devkit_path, 'VOC' + self._year)   # 'data/VOCdevkit2007/VOC2007'
        ...
        self._class_to_ind = dict(zip(self.classes, xrange(self.num_classes)))  # 将类名转换称下标的字典
        self._image_ext = '.jpg'
        self._image_index = self._load_image_set_index()   # ['000001','000003',……]_根据trainval.txt获取到的image索引_
    ...
    def _load_image_set_index(self):
        ...
        with open(image_set_file) as f:
            image_index = [x.strip() for x in f.readlines()]   # str
        return image_index
    ...
    def gt_roidb(self):
        ...
        gt_roidb = [self._load_pascal_annotation(index)
                    for index in self.image_index]  # 循环构建标注ground_truth
    ...
    def rpn_roidb(self):
        # 在经过RPN网络产生了proposal以后，这个函数作用是将这些proposal
        # 的
        # roi与groudtruth结合起来，送入网络训练。
        # 那怎么个结合法呢？proposal
        # 的
        # roidb格式与上面提到的gt_roidb一模一样，只不过overlap由1变成了与最接近的class的重合度。
        # 如何判断是最接近的class呢？每个proposal的box都与groud - truth的box做一次重合度计算，与anchor
        # _target
        # _layer.py中类似
        # overlap = (重合部分面积) / (proposal _box面积 + gt_boxes面积 - 重合部分面积)
        # 对于每个proposal，选出最大的那个gt
        # _boxes的值，然后填到相应的class
        # index下。
        if int(self._year) == 2007 or self._image_set != 'test':
            gt_roidb = self.gt_roidb()
    ...
    def _load_pascal_annotation(self, index):
        ...
        for ix, obj in enumerate(objs):
            ...
            boxes[ix, :] = [x1, y1, x2, y2]   # boxes为n*4矩阵，n表示一张图上有多少个objects，左上右下坐标
            gt_classes[ix] = cls
            overlaps[ix, cls] = 1.0
